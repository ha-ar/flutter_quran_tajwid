...
class TajweedFeedbackService {
  ...
  static String _normalizeForComparison(String text) {
    return text
        ...
        .replaceAll(
            RegExp(r'[\u0670]'), 'ا') // Dagger alif -> Normal Alif for diff
  ...
  static String? _checkLetterConfusion(String exp, String act) {
    ...
    final rules = {
      ...
      'ق': {
        'ك':
            "Ensure strict articulation of Qaf (ق) from the throat, distinguishing it from Kaf (ك)."
      },
      ...
      'ص': {
        'س':
            "Emphasize Sad (ص) with a full mouth (Tafkheem). Don't mix it with Sin (س)."
      },
      ...
      'ض': {
        'د':
            "Dad (ض) is unique; use the side of the tongue. It sounded like Dal (د).",
      ...
      'ح': {
        'ه':
            "Sharpen your Ha (ح) from the middle throat. It sounded like soft Haa (ه)."
      },
      'ه': {'ح': "Soften the Haa (ه) from the deepest throat."},

      // Ain vs Hamza
      'ع': {
        'ا': "Clarify the Ain (ع) from the middle throat.",
        'ء': "Clarify the Ain (ع) from the middle throat."
      },

      // Thal vs Zay/Sin
      'ذ': {
        'ز': "Thal (ذ) is interdental (tongue tip). Don't buzz match Zay (ز).",
      ...
      'ث': {
        'س':
            "Tha (ث) is interdental (tongue tip between teeth). It sounded like Sin (س)."
      }
    };
